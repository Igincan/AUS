...
namespace structures
{
	...
	{
		TreeNode<T>* copyNode = shallowCopy();
		for (int i = 0; i < degree(); i++)
		{
			TreeNode<T>* son = getSon(i);
			if (son != nullptr)
			{
				TreeNode<T>* copySon = son->deepCopy();
				copyNode->replaceSon(copySon, i);
			}
		}
		return copyNode;
	}
	...
	inline TreeNode<T>* TreeNode<T>::getBrother(int brothersOrder)
	{
		if (isRoot())
		{
			throw std::logic_error("TreeNode<T>::getBrother: Node has no parent.");
		}
	...
	{
		return isEmpty() ? 0 : root_->sizeOfSubtree();
	}
	...
	inline Tree<T>& Tree<T>::operator=(const Tree<T>& other)
	{
		...
		{
			clear();
			root_ = other.root_ == nullptr ? nullptr : other.root_->deepCopy();
		}
		return *this;
	}
	...
	inline bool Tree<T>::TreeIterator::operator!=(const Iterator<T>& other)
	{
		return
		!(
			path_->size() == dynamic_cast<const TreeIterator&>(other).path_->size() &&
			path_->size() > 0 &&
			path_->peek() == dynamic_cast<const TreeIterator&>(other).path_->peek() 
		);
	}
